...
public class Deneme2 {
	...
	public static void main(String[] args) {
		...
		List<Double> ortalamalar=students.stream().map(x->x.getNotlar().stream().mapToDouble(y->y.doubleValue()).sum()/x.getNotlar().size()).collect(Collectors.toList());
	...
	public void notOrtalamaHesaplama() {
		List<Double> ortalamalar = students.stream().map(x-> x.getNotlar()
		...
		Double ort2 = ortalamalar.stream().mapToDouble(x->x).average().getAsDouble();
